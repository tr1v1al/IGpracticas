...
TexturaXY::TexturaXY( const std::string & nom ): Textura(nom) {
   ...
   coefs_s[3] = 0.0f;
   coefs_t[0] = 0.0f;
   coefs_t[1] = -1.0f;   // Y, -1 por inversion vertical
   coefs_t[2] = 0.0f;
   coefs_t[3] = 0.0f;
}
...
void ColFuentesLuz::activar( )
{
   ...
   vector<vec4> pos_dir;
   int sz = vpf.size();
   float theta = 0.0f, phi = 0.0f, x = 0.0f, y = 0.0f, z = 0.0f;
   for (int i = 0; i < sz; ++i) {
      colores.push_back(vpf[i]->color);
      // Coordenadas esféricas a cartesianas
      // Parecido a la funcion Cartesianas de camara.cpp
      theta = radians(vpf[i]->longi);
      phi = radians(vpf[i]->lati);
      x = sin(theta)*cos(phi);
      y = sin(phi);
      z = cos(theta)*cos(phi);
      pos_dir.push_back({x, y, z, 0.0f});
   }
