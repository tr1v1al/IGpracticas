...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   if ( glfwGetKey( window, GLFW_KEY_L) == GLFW_PRESS )
   {
      ...
      // .....
      ColFuentesLuz * fuentes = apl->escenas[apl->ind_escena_act]->colFuentes();
      apl->revisualizar_escena = ProcesaTeclaFuenteLuz(fuentes, key);
   ...
   switch ( key )
   {
      ...
      case GLFW_KEY_Y :
         ...
         cout << "visualizacion de fbo " << (apl->visualizar_fbo ? "activada": "desactivada") << endl;
         break;
      case GLFW_KEY_T :
