...
{
   // Ajuste para solucionar problema de sensitividad
   float da1 = da/100.0f, db1 = db/100.0f;
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         org_polares += glm::vec3(da1, db1, 0.0f);
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         // .....
         org_polares += glm::vec3(da1, db1, 0.0f);
         glm::vec3 nuevas = Cartesianas(org_polares);
         punto_atencion -= nuevas - org_cartesianas;
         org_cartesianas = nuevas;
         actualizarEjesMCV() ;
         break ;
      }
      case ModoCam::prim_pers_despl :
      {
         ...
         punto_atencion += da1*eje[X] + db1*eje[Y];
         break ;
      }
   }
   matrices_actualizadas = false ;
...
void Camara3Modos::moverZ( const float dz )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         // .....
         float r_min = 0.1, epsilon = 0.03;
         org_polares[2] = r_min + (org_polares[2] - r_min) * pow(1.0f + epsilon, dz);
         org_cartesianas = Cartesianas(org_polares);
         break ;
      }
      case ModoCam::prim_pers_rot :
